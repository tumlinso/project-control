...
class ProjectReconciler:
    ...
    def _tasks(self) -> tuple[dict[str, dict[str, Any]], bool]:
        ...
        for raw in self.snapshot.todo_tables.get("tasks", []):
            ...
            terminal = status in {"done", "superseded", "cancelled", "canceled", "stale"}
    ...
    def reconcile(self) -> ReconciledProject:
        ...
        checkpoints = [dict(item) for item in semantic.get("checkpoints", [])] if semantic_available else [
            {
                **item,
                "effective_state": "reached" if item.get("state") == "reached" else "current_pending",
                "attention_eligible": item.get("state") != "reached",
                "relevance": "current_attention" if item.get("state") != "reached" else "reference",
                "relevance_reason": "legacy_raw_state_fallback",
            }
            for item in self.snapshot.todo_tables.get("checkpoints", [])
        ]
        gates = [dict(item) for item in semantic.get("gates", [])] if semantic_available else [
            {
                **item,
                "effective_state": "current_valid" if item.get("valid") else "current_failed",
                "attention_eligible": bool(item.get("required") and not item.get("valid")),
                "relevance": "current_attention" if item.get("required") and not item.get("valid") else "reference",
                "relevance_reason": "legacy_raw_state_fallback",
            }
        ...
        architectural_attention = rank_items([item for item in checkpoints if item.get("attention_eligible")])
