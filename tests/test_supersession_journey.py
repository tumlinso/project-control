...
repo=V2Repo()
try:
 repo.apply(base_plan([safe_task('OLD','src/old'),safe_task('NEW','src/new')]))
 ...
 def seed(conn,rev):
  ...
  for lane,run in [('OLD-LANE','OLD-RUN'),('NEW-LANE','NEW-RUN')]: conn.execute("INSERT INTO workflow_lanes(id,run_id,role,workspace_mode,state,created_at,updated_at,revision) VALUES(?,?, 'implementer','isolated_merge','ready','n','n',?)",(lane,run,rev))
  conn.execute("INSERT INTO workflow_lane_tasks(lane_id,position,task_id,state,enqueued_at,revision) VALUES('OLD-LANE',0,'OLD','queued','n',?)",(rev,)); conn.execute("INSERT INTO workflow_lane_tasks(lane_id,position,task_id,state,enqueued_at,revision) VALUES('NEW-LANE',0,'NEW','queued','n',?)",(rev,))
 ...
 with repo.service.db.read() as c: dispatch=c.execute("SELECT workspace_id FROM workflow_dispatches WHERE state='active'").fetchone()[0]
 assert dispatch==maintained['receipt']['preserved_workspace_ids'][0],(dispatch,maintained)
 print(json.dumps({'status':maintained['status'],'claimed':claimed['status']}))
finally: repo.close()
'''
        ...
        self.assertEqual(json.loads(result.stdout), {"status": "superseded", "claimed": "claimed"})
